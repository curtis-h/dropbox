...
<body>
    ...
    <script type="text/javascript">
                ...
                for(b in boxes) {
                    if(boxes.hasOwnProperty(b)) {
                        ...
                        if(google.maps.geometry.spherical.computeDistanceBetween(coord, ll) < (Math.sqrt(radius) * 100)) {
                            ...
                            boxes[b].marker.setMap(null);
                            boxes[b].marker = undefined;
                        }
                    }
                } 
            }
        ...
        function travel(coords) {
            $({countNum: 0}).animate({countNum: coords.length}, {
                duration: 8000,
                easing:'linear',
                step: function() {
                  var co = coords[Math.floor(this.countNum)];
                  globals.marker.setPosition(co);
                  globals.circle.setCenter(co);
                  globals.location = co;
        ...
        function makeMap(location) {
          ...
          globals.circle = new google.maps.Circle(circleOptions);
          
          google.maps.event.addListener(map, 'click', function(event) {
              var dest = new google.maps.LatLng(event.latLng.lat(), event.latLng.lng());
              var request = {
                  origin: globals.location,
                  destination: dest,
